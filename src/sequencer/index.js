...
const states = { STATE_SEQ_IDLE: 'STATE_SEQ_IDLE', STATE_SEQ_RUNNING: 'STATE_SEQ_RUNNING' } 
module.exports = (sequence, socket) => {
  ...
  Object.keys(taskObjects).map((id) => {
    ...
    sequence.nodes[id].predecessors.map(dependencyId => {
        if (dependencyId != 'startNode') task.addDependency(taskObjects[dependencyId]);
    }) 
  });
  return {
    sequence, dependencyGraph: taskObjects, state: states.STATE_SEQ_IDLE,
    run(){
      socket.emit("sequencerState", {state: states.STATE_SEQ_RUNNING});
      sequence.startNodes.map((id) => { const task = this.dependencyGraph[id];
        task.run();
      });
    },
  };
